...
class DeformableDETRHeadIncreaseLossDecouple(DETRHead):
    ...
    #          enc_outputs_class: Tensor, enc_outputs_coord: Tensor,
    #          batch_data_samples: SampleList, head_inputs_ori_dict: dict,
    #          ori_result, confidence_shred, ori_num_queries) -> dict:
    #     batch_gt_instances = []
    #     batch_img_metas = []
    #     for data_sample in batch_data_samples:
    #         batch_img_metas.append(data_sample.metainfo)
    ...
    #     # print("=============================")
    #     # assert 1 < 0
    #     device = batch_gt_instances[0].labels.device
    #     type = batch_gt_instances[0].labels.dtype
    #
    #     batch_gt_instances_ori = []
    #     # ori_not_blank_samples = []
    #     for result in ori_result:
    #         batch_gt_instance = InstanceData()
    #         batch_gt_instance.set_data({
    #             'labels': torch.Tensor([]).type(type).to(device),
    #             'bboxes': torch.empty(0, 4).type(type).to(device)
    #         })
    #         # print(batch_gt_instance)
    #         # batch_gt_instance.bboxes = torch.tensor([[]], dtype=)
    #         for bbox in result:
    #             if bbox['scores'] > confidence_shred:
    #                 bbox.__delattr__('scores')
    #                 # print('cat')
    #                 batch_gt_instance = InstanceData.cat([bbox, batch_gt_instance])
    #             else:
    #                 break
    #         # print(batch_gt_instance)
    #         batch_gt_instances_ori.append(batch_gt_instance)
    #     # print("===========batch_gt_instance after================")
    #     # print(batch_gt_instances)
    #
    #     all_layers_outputs_classes, all_layers_outputs_coords = self(hidden_states, references)
    #
    #     loss_inputs_ori = (all_layers_outputs_classes[:, :, :ori_num_queries],
    #                        all_layers_outputs_coords[:, :, :ori_num_queries],
    #                        enc_outputs_class,
    #                        enc_outputs_coord,
    #                        batch_gt_instances_ori,
    #                        batch_img_metas)
    #
    #     loss_inputs_new = (all_layers_outputs_classes[:, :, ori_num_queries:],
    #                        all_layers_outputs_coords[:, :, ori_num_queries:],
    #                        enc_outputs_class,
    #                        enc_outputs_coord,
    #                        batch_gt_instances,
    #                        batch_img_metas)
    #
    #     losses_ori = self.loss_by_feat(*loss_inputs_ori)
    #     losses_new = self.loss_by_feat(*loss_inputs_new)
    #     # assert 1 < 0
    #     losses = {}
    #     for key in losses_ori:
    #         losses[key] = losses_ori[key] + losses_new[key]
    #     return losses
    ...
    def loss_by_feat_decouple_(
        ...
        losses_cls_old, losses_cls_new, losses_bbox, losses_iou = multi_apply(
        ...
        num_dec_layer = 0
        for loss_cls_i_old, loss_cls_i_new, loss_bbox_i, loss_iou_i in \
                zip(losses_cls_old[:-1], losses_cls_new[:-1], losses_bbox[:-1], losses_iou[:-1]):
    ...
    def loss_by_feat_single_decouple(self,
        ...
        # print("label_weights_list:", label_weights_list)
        # print("bbox_targets_list:", bbox_targets_list)
        # print("bbox_weights_list:", bbox_weights_list)
        ...
        # print("all:{}, old:{}, new:{}".format(loss_cls, loss_cls_old, loss_cls_new))
        ...
        return loss_cls_old, loss_cls_new, loss_bbox, loss_iou
