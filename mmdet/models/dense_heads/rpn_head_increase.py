...
class RPNHeadIncrease(AnchorHead):
    ...
    def loss_and_predict_decompose(
        self,
        x: Tuple[Tensor],
        batch_data_samples: SampleList,
        proposal_cfg: Optional[ConfigDict] = None,
        ...
        loss_inputs = outs + (batch_gt_instances, batch_img_metas,
                              batch_gt_instances_ignore, ori_class_num)
        losses = self.loss_by_feat_decompose(*loss_inputs)
    ...
    def loss_by_feat_decompose(self,
                     cls_scores: List[Tensor],
                     bbox_preds: List[Tensor],
                     batch_gt_instances: InstanceList,
                     batch_img_metas: List[dict],
        ...
        Returns:
            dict[str, Tensor]: A dictionary of loss components.
        """
        losses = self.loss_by_feat_decompose(
            cls_scores,
            bbox_preds,
            batch_gt_instances,
            batch_img_metas,
            batch_gt_instances_ignore=batch_gt_instances_ignore,
            ori_class_num=ori_class_num
        )
        return dict(
            loss_rpn_cls=losses['loss_cls'], loss_rpn_bbox=losses['loss_bbox'])

    def loss_by_feat_decompose(
            self,
            cls_scores: List[Tensor],
            bbox_preds: List[Tensor],
            batch_gt_instances: InstanceList,
