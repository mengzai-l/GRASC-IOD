...
class BBoxHeadIncrease(BaseModule):
    ...
    def _get_targets_single_weight(self, pos_priors: Tensor, neg_priors: Tensor,
                            pos_gt_bboxes: Tensor, pos_gt_labels: Tensor,
                                   pos_gt_reg_weight: List,
                                   pos_gt_cls_weight: List,
                            cfg: ConfigDict,) -> tuple:
        """Calculate the ground truth for proposals in the single image
        according to the sampling results.
        ...
        if num_pos > 0:
            ...
            # print(pos_gt_weight)
            # assert 1 < 0
            label_weights[:num_pos] = torch.Tensor(pos_gt_cls_weight).to(label_weights.device)
            if not self.reg_decoded_bbox:
                pos_bbox_targets = self.bbox_coder.encode(
                    pos_priors, pos_gt_bboxes)
            else:
                ...
                pos_bbox_targets = get_box_tensor(pos_gt_bboxes)
            bbox_targets[:num_pos, :] = pos_bbox_targets
            bbox_weights[:num_pos, :] = torch.Tensor(pos_gt_reg_weight).to(label_weights.device).unsqueeze(dim=1)
        if num_neg > 0:
            label_weights[-num_neg:] = 1.0
    ...
    def get_targets_weight(self,
                    sampling_results: List[SamplingResult],
                    rcnn_train_cfg: ConfigDict,
                    concat: bool = True,
                    pseudo_label_cls_weights_list=None,
                    pseudo_label_reg_weights_list=None,
                    gt_pseudo_num=None,
                    gt_new_num_list=None,
                           ori_num_classes=None,
                           num_classes=None,
                           ) -> tuple:
        """Calculate the ground truth for all samples in a batch according to
        the sampling_results.
        ...
        pos_gt_bboxes_list = [res.pos_gt_bboxes for res in sampling_results]
        pos_gt_labels_list = [res.pos_gt_labels for res in sampling_results]
        pos_gt_reg_weight_list = []
        pos_gt_cls_weight_list = []
        for res, pseudo_label_reg_weights, pseudo_label_cls_weights, gt_new_num in (
                zip(sampling_results, pseudo_label_reg_weights_list, pseudo_label_cls_weights_list, gt_new_num_list)):
            pos_assigned_gt_inds = res.pos_assigned_gt_inds
            pos_reg_gt_weight = []
            pos_cls_gt_weight = []
            for inds in pos_assigned_gt_inds:
                if inds < gt_new_num:
                    pos_reg_gt_weight.append(1.)
                    pos_cls_gt_weight.append(1.)
                else:
                    pos_reg_gt_weight.append(pseudo_label_reg_weights[inds-gt_new_num])
                    pos_cls_gt_weight.append(pseudo_label_cls_weights[inds-gt_new_num])
            pos_gt_reg_weight_list.append(pos_reg_gt_weight)
            pos_gt_cls_weight_list.append(pos_cls_gt_weight)
        labels, label_weights, bbox_targets, bbox_weights = multi_apply(
            self._get_targets_single_weight,
            pos_priors_list,
            neg_priors_list,
            pos_gt_bboxes_list,
            pos_gt_labels_list,
    ...
    def loss_and_target_weight(self,
                        cls_score: Tensor,
                        bbox_pred: Tensor,
                        rois: Tensor,
                        sampling_results: List[SamplingResult],
        ...
        cls_reg_targets = self.get_targets_weight(
            sampling_results,
            rcnn_train_cfg,
            concat=concat,
            pseudo_label_cls_weights_list=pseudo_label_cls_weights_list,
            pseudo_label_reg_weights_list=pseudo_label_reg_weights_list,
            gt_pseudo_num=gt_pseudo_num,
            gt_new_num_list=gt_new_num,
    ...
    def loss_weight(self,
        ...
        # ori_num_classes=10
        # num_classes=20
        # print("ori_num_classes:{}, num_classes:{}".format(ori_num_classes, num_classes))
        losses = dict()
        label_mask_for_old_positive = labels < ori_num_classes
        # print(label_mask_for_old_positive)
        label_mask_for_new_positive = torch.FloatTensor(
            np.uint8(labels.cpu() >= ori_num_classes) & np.uint8(labels.cpu() < num_classes)).to(labels.device)
        label_mask_for_new_negative = (labels == num_classes)

        # print("ori_num_class:{}, num_classes:{}".format(ori_num_classes, num_classes))
        # assert 1 < 0

        label_weights_for_old_positive = torch.mul(label_weights, label_mask_for_old_positive)
        label_weights_for_new_positive = torch.mul(label_weights, label_mask_for_new_positive)
        label_weights_for_new_negative = torch.mul(label_weights, label_mask_for_new_negative)

        if cls_score is not None:
            avg_factor = max(torch.sum(label_weights).float().item(), 1.)
            if cls_score.numel() > 0:
                loss_cls_old_positive_ = self.loss_cls(
                    cls_score,
                    labels,
                    label_weights_for_old_positive,
                    avg_factor=avg_factor,
                    reduction_override=reduction_override)
                loss_cls_new_positive_ = self.loss_cls(
                    cls_score,
                    labels,
                    label_weights_for_new_positive,
                    avg_factor=avg_factor,
                    reduction_override=reduction_override)
                loss_cls_new_negative_ = self.loss_cls(
                    cls_score,
                    labels,
                    label_weights_for_new_negative,
                    avg_factor=avg_factor,
                    reduction_override=reduction_override)
                # print("cls losses all: {}, old: {}, new:{}, new: {}".format(loss_cls_, loss_cls_old_positive_, loss_cls_new_positive_, loss_cls_new_negative_))

                if isinstance(loss_cls_old_positive_, dict):
                    losses.update(loss_cls_old_positive_)
                    losses.update(loss_cls_new_positive_)
                    losses.update(loss_cls_new_negative_)
                else:
                    losses['loss_cls_old_positive_'] = loss_cls_old_positive_
                    losses['loss_cls_new_positive_'] = loss_cls_new_positive_
                    losses['loss_cls_new_negative_'] = loss_cls_new_negative_
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                bbox_weights_old_positive = label_weights_for_old_positive.unsqueeze(dim=1).expand(-1, 4)
                bbox_weights_new_positive = label_weights_for_new_positive.unsqueeze(dim=1).expand(-1, 4)

                losses['loss_bbox_old_positive'] = self.loss_bbox(
                    pos_bbox_pred,
                    bbox_targets[pos_inds.type(torch.bool)],
                    bbox_weights_old_positive[pos_inds.type(torch.bool)],
                    avg_factor=bbox_targets.size(0),
                    reduction_override=reduction_override)
                losses['loss_bbox_new_positive'] = self.loss_bbox(
                    pos_bbox_pred,
                    bbox_targets[pos_inds.type(torch.bool)],
                    bbox_weights_new_positive[pos_inds.type(torch.bool)],
                    avg_factor=bbox_targets.size(0),
                    reduction_override=reduction_override)
            else:
    ...
    def loss(self,
        ...
        """
        losses = dict()
        label_weights_for_old_positive = labels < 10
        label_weights_for_new_positive = torch.FloatTensor(
            np.uint8(labels.cpu() >= 10) & np.uint8(labels.cpu() < 20)).to(labels.device)
        label_weights_for_new_negative = (labels == 20)
        ...
        if bbox_pred is not None:
            ...
            if pos_inds.any():
                ...
                #     avg_factor=bbox_targets.size(0),
                #     reduction_override=reduction_override)
                # print("loss_bbox_old_positive: {}, loss_bbox_new_positive: {}, all: {}".
                #       format(losses['loss_bbox_old_positive'], losses['loss_bbox_new_positive'], losses['loss_bbox']))
            else:
                losses['loss_bbox'] = bbox_pred[pos_inds].sum()
