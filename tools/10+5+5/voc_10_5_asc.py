...
class ASCCalibrator:
    def __init__(self, config_file, checkpoint_file, rigid_classes, base_classes, device="cuda:0"):
        ...
        if not os.path.exists(checkpoint_file): raise FileNotFoundError(f"Weights not found: {checkpoint_file}")
    ...
    def process_folder(self, v4_ann_dir, v4_img_dir):
        ...
        for xml_p in tqdm(xml_files, desc="ASC Calibration Progress"):
            ...
            if not img_p.exists(): os.remove(xml_p); continue
            ...
            try:
                ...
                for obj in root.findall("object"):
                    ...
                    if name in self.rigid_classes:
                        if len(preds) == 0: has_mod = True; continue
                        bnd = obj.find("bndbox")
